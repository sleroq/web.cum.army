...
const Player = (props: PlayerProps) => {
	...
	const debugAutoplay = (...args: unknown[]) => console.debug("[PlayerAutoplay]", streamKey, ...args);
	...
	const scheduleAutoplayAttempts = (preferSound: boolean) => {
		...
		const attempt = async (isFinalAttempt: boolean, reason: string) => {
			...
			if (!el.paused) {
				debugAutoplay("attempt:skip:already-playing", {scheduleToken, reason, ...describeVideoEl(el)});
				setAutoplayFailed(false);
				return;
			}

			debugAutoplay("attempt:start", {scheduleToken, reason, isFinalAttempt, ...describeVideoEl(el)});
			...
			if (isFinalAttempt) {
				debugAutoplay("attempt:final:autoplayFailed=true", {scheduleToken, reason, ...describeVideoEl(el)});
